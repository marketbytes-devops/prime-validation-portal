...
export default function CertificateList() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-primary uppercase tracking-tight">Certificate Numbers</h2>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-8 shadow-xl shadow-slate-200/20 flex flex-wrap gap-4 items-end">
        <div className="flex-1 min-w-[200px]">
          ...
          <div className="relative">
            <input type="text" value={filters.search} placeholder="ID or Number…"
              ...
              className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-2.5 pl-10 outline-none focus:border-primary focus:bg-white transition-all font-semibold text-sm" />
        ...
        <div className="w-full sm:w-auto">
          ...
          <select value={filters.status} onChange={e => setF('status', e.target.value)}
            className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-2.5 outline-none focus:border-primary focus:bg-white transition-all font-semibold text-sm min-w-[140px]">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl shadow-slate-200/50 overflow-hidden">
        ...
        {loading ? (
          <div className="py-20 text-center">
            <FaSpinner className="animate-spin w-8 h-8 text-primary mx-auto" />
          </div>
        ) : certs.length === 0 ? (
                  ...
                  <tr key={cert.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[9px] font-bold uppercase tracking-widest
                        ${cert.is_expired ? 'bg-red-50 text-secondary border border-red-100' : 'bg-emerald-50 text-emerald-600 border border-emerald-100'}`}>
                         <span className={`w-1.5 h-1.5 rounded-full ${cert.is_expired ? 'bg-secondary' : 'bg-emerald-500'}`} />
                        {cert.is_expired ? 'Expired' : 'Active'}
                      </span>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex gap-2">
                        <button onClick={() => handleEditOpen(cert)} className="p-2.5 rounded-xl text-primary hover:bg-slate-100 transition-all border border-slate-200">
                          <FaEdit className="w-4 h-4" />
                        </button>
                        <button onClick={() => setDelId(cert.id)} className="p-2.5 rounded-xl text-secondary hover:bg-red-50 transition-all border border-red-100">
        ...
        {/* Pagination Controls */}
        {!loading && certs.length > 0 && totalItems > itemsPerPage && (
          <div className="bg-slate-50/50 px-6 py-4 border-t border-slate-200 flex items-center justify-between">
            <p className="text-xs font-semibold text-slate-500">
              Showing <span className="text-primary">{(page - 1) * itemsPerPage + 1}</span> to <span className="text-primary">{Math.min(page * itemsPerPage, totalItems)}</span> of <span className="text-primary">{totalItems}</span> records
            </p>
            <div className="flex gap-2">
              <button
                disabled={page === 1}
                onClick={() => setPage(p => p - 1)}
                className="p-2 rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-primary hover:border-primary transition-all disabled:opacity-40"
              >
              ...
              {Array.from({ length: Math.ceil(totalItems / itemsPerPage) }, (_, i) => i + 1).map(p => (
                <button
                  key={p}
                  onClick={() => setPage(p)}
                  className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${page === p ? 'bg-primary text-white shadow-lg' : 'bg-white border border-slate-200 text-slate-400 hover:text-primary'}`}
                >
                  {p}
                </button>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={handleEditOutsideClick}>
          <div ref={editModalRef} className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 border border-slate-200">
            ...
            <div className="space-y-4">
              ...
              <textarea placeholder="Description…" value={editForm.description} onChange={e => setEditForm(f => ({ ...f, description: e.target.value }))} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 text-sm font-semibold h-24 resize-none outline-none focus:border-primary transition-colors" />
              <div className="flex gap-3 pt-2">
                <button onClick={handleEditSave} disabled={saving} className="flex-1 py-3.5 rounded-xl bg-primary text-white font-bold text-xs uppercase tracking-widest shadow-lg shadow-blue-900/20">{saving ? 'Updating…' : 'Save Changes'}</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={handleDelOutsideClick} >
          <div ref={delModalRef} className="w-full max-w-sm bg-white rounded-3xl shadow-2xl p-8 text-center border border-slate-200">
            <div className="w-16 h-16 rounded-3xl bg-red-50 flex items-center justify-center mx-auto mb-6 border border-red-100">
              <FaExclamationTriangle className="w-8 h-8 text-secondary" />
            </div>
            ...
            <div className="flex flex-col gap-2">
              <button onClick={() => handleDelete(delId)} className="w-full py-4 rounded-2xl bg-secondary text-white font-bold text-xs uppercase tracking-widest shadow-lg shadow-red-900/20">Confirm Delete</button>
...
const ModalInput = ({ label, value, onChange, type = 'text', red }) => (
  <div>
    <label className={`block text-[10px] font-bold uppercase tracking-widest mb-1.5 ${red ? 'text-secondary' : 'text-slate-600'}`}>{label}</label>
    <input type={type} value={value} onChange={e => onChange(e.target.value)}
      className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-2.5 text-sm font-semibold outline-none focus:border-primary focus:bg-white transition-all" />
  </div>
)
